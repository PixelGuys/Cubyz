...
pub fn increaseTimerResolution() void {
    ...
    std.log.info("Set system timer resolution: {d}ms -> {d}ms.", .{initialResolution, currentResolution});
}
...
pub fn resetTimerResolution() void {
    ...
    std.log.info("Reset system timer resolution: {d}ms.", .{initialResolution});
}
...
pub fn preciseSleep(sleepDuration: std.Io.Duration) void {
    ...
    if(start.durationTo(end).nanoseconds - sleepDuration.nanoseconds > currentResolution * 100) {
		const desired = @as(f64, @floatFromInt(sleepDuration.nanoseconds - 100 * currentResolution)) / 1_000_000;
		const actual = @as(f64, @floatFromInt(start.durationTo(end).nanoseconds - 100 * currentResolution)) / 1_000_000;
        std.log.warn("OOverslept: Desired sleep: {d:.1}ms, Actual sleep: {d:.1}ms, Timer resolution: {d}", .{desired, actual, currentResolution});
    }
