...
pub fn render(playerPosition: Vec3d, deltaTime: f64) void {
	...
	const ambient = @max(nightColor*@as(Vec3f, @splat(settings.nightBrightness)), @as(Vec3f, @splat(game.world.?.dayCycle.ambientLight)));

	itemdrop.ItemDisplayManager.update(deltaTime);
	renderWorld(game.world.?, ambient, game.world.?.dayCycle.fog.skyColor, playerPosition);
...
pub fn renderWorld(world: *World, ambientLight: Vec3f, skyColor: Vec3f, playerPos: Vec3d) void { // MARK: renderWorld()
	...
	if (!blocks.meshes.hasFog(playerBlock)) {
		c.glUniform3fv(deferredUniforms.@"fog.color", 1, @ptrCast(&game.world.?.dayCycle.fog.fogColor));
		c.glUniform1f(deferredUniforms.@"fog.density", game.world.?.dayCycle.fog.density);
		c.glUniform1f(deferredUniforms.@"fog.fogLower", game.world.?.dayCycle.fog.fogLower);
		c.glUniform1f(deferredUniforms.@"fog.fogHigher", game.world.?.dayCycle.fog.fogHigher);
	} else {
...
const Bloom = struct { // MARK: Bloom
	...
	fn extractImageDataAndDownsample(playerBlock: blocks.Block, playerPos: Vec3d, viewMatrix: Mat4f) void {
		...
		buffer1.bind();
		if (!blocks.meshes.hasFog(playerBlock)) {
			c.glUniform3fv(colorExtractUniforms.@"fog.color", 1, @ptrCast(&game.world.?.dayCycle.fog.fogColor));
			c.glUniform1f(colorExtractUniforms.@"fog.density", game.world.?.dayCycle.fog.density);
			c.glUniform1f(colorExtractUniforms.@"fog.fogLower", game.world.?.dayCycle.fog.fogLower);
			c.glUniform1f(colorExtractUniforms.@"fog.fogHigher", game.world.?.dayCycle.fog.fogHigher);
		} else {
			const fogColor = blocks.meshes.fogColor(playerBlock);
...
pub const Skybox = struct {
	...
	pub fn render() void {
		...
		const starOpacity: f32 = game.world.?.dayCycle.getStarOpacity();

		if (starOpacity != 0) {
			starPipeline.bind(null);

			const starMatrix = game.projectionMatrix.mul(viewMatrix.mul(Mat4f.rotationX(2*std.math.pi*game.world.?.dayCycle.getDayProgress())));
