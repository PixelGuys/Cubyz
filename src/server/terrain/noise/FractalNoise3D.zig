...
pub fn generateAligned(allocator: Allocator, wx: i32, wy: i32, wz: i32, voxelSize: u31, width: u31, height: u31, depth: u31, worldSeed: u64, scale: u31) !Array3D(f32) {
	std.debug.assert(wx & scale-1 == 0 and wy & scale-1 == 0 and wz & scale-1 == 0); // Alignment;
	std.debug.assert(width-1 & scale-1 == 0 and height-1 & scale-1 == 0 and depth-1 & scale-1 == 0); // dimensions need to be of the form n*scale + 1 with n ∈ ℕ \ {0}
	std.debug.assert(width > 1 and height > 1 and depth > 1); // dimensions need to be of the form n*scale + 1 with n ∈ ℕ \ {0}
	const map = try Array3D(f32).init(allocator, width, height, depth);
	...
	while(x0 < width) : (x0 += scaledScale) {
		var y0: u31 = 0;
		while(y0 < height) : (y0 += scaledScale) {
			var z0: u31 = 0;
...
fn generateInitializedFractalTerrain(wx: i32, wy: i32, wz: i32, startingScale: u31, worldSeed: u64, bigMap: Array3D(f32), maxResolution: u31) void {
	...
	while(res != 0) : (res /= 2) {
		...
		while(x < bigMap.width) : (x += 2*res) {
			var y: u31 = res;
			while(y+res < bigMap.height) : (y += 2*res) {
				var z: u31 = 0;
