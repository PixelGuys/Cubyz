...
pub fn generate(map: *LargeStructureMapFragment, worldSeed: u64) void { // TODO: Why is this so slow in debug mode? Is it my laptop or some other factor?
	...
	var x = map.pos[0] -% margin[0];
	while (x != map.pos[0] +% mapSize[0] +% margin[0]) : (x += 32) {
		var y = map.pos[1] -% margin[1];
		while (y != map.pos[1] +% mapSize[1] +% margin[1]) : (y += 32) {
			const biome = biomeMap.getSurfaceBiome(x, y);
			const biomePos: Vec3i = .{x + 16, y + 16, biomeMap.getSurfaceHeight(x, y)};
			const distance = map.pos -% biomePos;
			if (@reduce(.Or, distance +% mapSize < biome.maxSdfExtend.min)) continue;
			if (@reduce(.Or, distance > biome.maxSdfExtend.max)) continue;
			var seed = main.random.initSeed3D(worldSeed, biomePos);
			for (biome.caveSdfModels) |sdfModel| {
