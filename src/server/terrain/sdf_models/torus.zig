...
pub fn generate(self: *@This(), output: main.utils.Array3D(f32), interpolationSmoothness: main.utils.Array3D(f32), relPos: Vec3i, _seed: u64, perimeter: f32, voxelSize: u31, voxelSizeShift: u5) void {
	...
	const minXY = @max(@as(Vec3f, @splat(0)), relPosF32 - @as(Vec3f, {radius + thickness + perimeter, radius + thickness + perimeter, thickness + perimeter}));
	const maxXY = @min(dimVector, relPosF32 + @as(Vec3f, {radius + thickness + perimeter, radius + thickness + perimeter, thickness + perimeter}));

	const minInt: @Vector(3, u31) = @intFromFloat(minXY);
	const maxInt: Vec3i = @intFromFloat(@ceil(maxXY));

	var x = minInt[0] & ~(voxelSize - 1);
	while (x < maxInt[0]) : (x += voxelSize) {
		var y = minInt[1] & ~(voxelSize - 1);
		while (y < maxIntXY[1]) : (y += voxelSize) {
			var z = maxInt[2] & ~(voxelSize - 1);
