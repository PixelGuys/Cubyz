...
fn generateSphere(output: Array3D(f32), rx: i32, ry: i32, rz: i32, radius: i32, voxelSize: u31, voxelSizeShift: u5) void {
	const minX = @max(0, rx - radius);
	const maxX = @min(output.width*voxelSize, rx + radius);
	var x = minX;
	while (x < maxX) : (x += voxelSize) {
		const minY = @max(0, ry - radius);
		const maxY = @min(output.depth*voxelSize, ry + radius);
		var y = minY;
		while (y < maxY) : (y += voxelSize) {
			const minZ = @max(0, rz - radius);
			const maxZ = @min(output.height*voxelSize, rz + radius);
			var z = minZ;
			while (z < maxZ) : (z += voxelSize) {
				const distanceSquare = (x - rx)*(x - rx) + (y - ry)*(y - ry) + (z - rz)*(z - rz);
				if (distanceSquare > radius*radius) continue;
				if (z - rz < 0) continue;
				const signedDistanceSquare: f32 = @floatFromInt(@min(distanceSquare - radius*radius, std.math.sign(z - rz)*(z - rz)*(z - rz)));
				output.ptr(x >> voxelSizeShift, y >> voxelSizeShift, z >> voxelSizeShift).* -= 10*signedDistanceSquare/@as(f32, @floatFromInt(radius*radius));
			}
		}
	}
}
