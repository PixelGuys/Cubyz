...
pub const Biome = struct { // MARK: Biome
	...
	pub fn init(self: *Biome, id: []const u8, paletteId: u32, zon: ZonElement) void {
		...
		if(self.radius < terrain.SurfaceMap.MapFragment.biomeSize/2) {
			std.log.err("Biome {s} has radius {d}, smaller than grid resolution. It should be at least {d}", .{self.id, self.radius, terrain.SurfaceMap.MapFragment.biomeSize/2});
		}
