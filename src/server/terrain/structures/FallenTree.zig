...
pub fn generateFallen(self: *FallenTree, x: i32, y: i32, z: i32, length: u32, chunk: *main.chunk.ServerChunk, caveMap: terrain.CaveMap.CaveMapView, seed: *u64) void {
	...
	for (0..4) |i| {
		const dir: i32 = @intCast(i);

		var dx: i32 = 0;
		var dy: i32 = 0;

		if (dir == 0 and chunk.liesInChunk(x + 1, y, z)) {
			dx = 1;
		} else if (dir == 1 and chunk.liesInChunk(x - 1, y, z)) {
			dx = -1;
		} else if (dir == 2 and chunk.liesInChunk(x, y + 1, z)) {
			dy = 1;
		} else if (dir == 3 and chunk.liesInChunk(x, y - 1, z)) {
			dy = -1;
		}
