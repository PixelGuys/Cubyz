...
const ChunkManager = struct { // MARK: ChunkManager
	...
	const ChunkLoadTask = struct { // MARK: ChunkLoadTask
		...
		pub fn getPriority(self: *ChunkLoadTask) f32 {
			switch(self.source) {
				.user => |user| return self.pos.getPriority(user.getPosition()),
	...
	const LightMapLoadTask = struct { // MARK: LightMapLoadTask
		...
		pub fn getPriority(self: *LightMapLoadTask) f32 {
			if(self.source) |user| {
				return self.pos.getPriority(user.getPosition(), terrain.LightMap.LightMapFragment.mapSize) + 100;
			} else {
...
pub const ServerWorld = struct { // MARK: ServerWorld
	...
	pub fn findPlayer(self: *ServerWorld, user: *User) void {
		...
		if(playerData == .null) {
			user.setPosition(@floatFromInt(self.spawn));

			main.items.Inventory.Sync.setGamemode(user, self.defaultGamemode);
		} else {
			main.ecs.component_list.entity.fromZon(main.globalAllocator, user.id, main.ecs.getTypeById("cubyz:player"), playerData.getChild("entity"));
	...
	pub fn savePlayer(self: *ServerWorld, user: *User) !void {
		...
		playerZon.put("entity", main.ecs.component_list.entity.toZon(main.stackAllocator, user.id));
		playerZon.put("gamemode", @tagName(user.gamemode.load(.monotonic)));

		{
			main.items.Inventory.Sync.ServerSide.mutex.lock();
			defer main.items.Inventory.Sync.ServerSide.mutex.unlock();
			if(main.items.Inventory.Sync.ServerSide.getInventoryFromSource(.{.playerInventory = @intFromEnum(user.id)})) |inv| {
				playerZon.put("playerInventory", inv.save(main.stackAllocator));
			}

			if(main.items.Inventory.Sync.ServerSide.getInventoryFromSource(.{.hand = @intFromEnum(user.id)})) |inv| {
