...
fn handleGroupChanges(op: Operation, groupName: []const u8, split: *std.mem.SplitIterator(u8, .scalar), source: *User) void {
	...
	switch (op) {
		...
		.delete => {
			if (!permission.deleteGroup(main.stackAllocator, groupName)) {
