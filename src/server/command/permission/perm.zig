	...
	if (split.next()) |arg| {
		if (std.ascii.eqlIgnoreCase(arg, "remove")) {
			...
			if (target.user.hasPermission(permissionPath)) {
				source.sendMessage("#00ff00Player {f}§#00ff00 has permission for path: {s}", .{target.user, permissionPath});
			} else {
				source.sendMessage("#ff0000Player {f}§#ff0000 has no permission for path: {s}", .{target.user, permissionPath});
			}
		}
	}
}
