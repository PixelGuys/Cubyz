...
var _bodyDensity: [maxBlockCount]f32 = undefined;
...
pub fn register(_: []const u8, id: []const u8, zon: ZonElement) u16 {
	...
	_bodyDensity[size] = zon.get(f32, "bodyDensity", 0.7);
...
pub const Block = packed struct { // MARK: Block
	...
	/// For buoyancy and other fluid mechanics.
	pub inline fn bodyDensity(self: Block) f32 {
		return _bodyDensity[self.typ];
	}
