...
pub const BaseItem = struct { // MARK: BaseItem
    ...
    fn init(self: *BaseItem, allocator: NeverFailingAllocator, texturePath: []const u8, replacementTexturePath: []const u8, id: []const u8, zon: ZonElement) void {
        ...
        var SplitName = std.mem.splitAny(u8, self.name, ":");
        const ItemAddon = SplitName.next() orelse "unknown";
        const ItemName = SplitName.next() orelse "unknown";
        tooltip.writer().print(
            \\{s} #808080from: {s}#FFFFFF
        , .{ ItemName, ItemAddon }) catch unreachable;
        tooltip.append('\n');
...
pub const Tool = struct { // MARK: Tool
    ...
    fn getTooltip(self: *Tool) []const u8 {
        ...
        var SplitIter = std.mem.splitAny(u8, self.type.id(), ":");
        const AddonSource = SplitIter.next() orelse "unknown";
        const ToolName = SplitIter.next() orelse "unknown";
