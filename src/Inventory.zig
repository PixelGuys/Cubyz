...
pub const Command = struct { // MARK: Command
	...
	fn executeBaseOperation(self: *Command, allocator: NeverFailingAllocator, _op: BaseOperation, side: Side) void { // MARK: executeBaseOperation()
		...
		switch(op) {
			...
			.addHealth => |*info| {
				if(side == .server) {
					info.previous = info.target.?.getHealth().health;

					info.target.?.getHealth().health = std.math.clamp(info.target.?.getHealth().health + info.health, 0, info.target.?.getHealth().maxHealth);

					if(info.target.?.getHealth().health <= 0) {
						info.target.?.getHealth().health = info.target.?.getHealth().maxHealth;
						info.cause.sendMessage(info.target.?.getName().name);
			...
			.addEnergy => |*info| {
				if(side == .server) {
					info.previous = info.target.?.getEnergy().energy;

					info.target.?.getEnergy().energy = std.math.clamp(info.target.?.getEnergy().energy + info.energy, 0, info.target.?.getEnergy().maxEnergy);
		...
		fn run(self: Drop, allocator: NeverFailingAllocator, cmd: *Command, side: Side, user: ?*main.server.User, _: Gamemode) error{serverFailure}!void {
			...
			if(self.source.inv.type == .crafting) {
				...
				if(_items[0].item != null) {
					if(side == .server) {
						const direction = vec.rotateZ(vec.rotateX(Vec3f{0, 1, 0}, -user.?.getTransform().rot[0]), -user.?.getTransform().rot[2]);
						main.server.world.?.dropWithCooldown(_items[0], user.?.getTransform().pos, direction, 20, main.server.updatesPerSec*2);
					}
				}
				return;
			}
			...
			if(side == .server) {
				const direction = vec.rotateZ(vec.rotateX(Vec3f{0, 1, 0}, -user.?.getTransform().rot[0]), -user.?.getTransform().rot[2]);
				main.server.world.?.dropWithCooldown(.{.item = self.source.ref().item.?.clone(), .amount = amount}, user.?.getTransform().pos, direction, 20, main.server.updatesPerSec*2);
			}
	...
	const DepositOrDrop = struct { // MARK: DepositOrDrop
		...
		pub fn run(self: DepositOrDrop, allocator: NeverFailingAllocator, cmd: *Command, side: Side, user: ?*main.server.User, _: Gamemode) error{serverFailure}!void {
			...
			outer: for(sourceItems, 0..) |*sourceStack, sourceSlot| {
				...
				if(side == .server) {
					const direction = vec.rotateZ(vec.rotateX(Vec3f{0, 1, 0}, -user.?.getTransform().rot[0]), -user.?.getTransform().rot[2]);
					main.server.world.?.drop(sourceStack.clone(), user.?.getTransform().pos, direction, 20);
				}
				cmd.executeBaseOperation(allocator, .{.delete = .{
					.source = .{.inv = self.source, .slot = @intCast(sourceSlot)},
	...
	const AddHealth = struct { // MARK: AddHealth
		...
		pub fn run(self: AddHealth, allocator: NeverFailingAllocator, cmd: *Command, side: Side, _: ?*main.server.User, _: Gamemode) error{serverFailure}!void {
			...
			cmd.executeBaseOperation(allocator, .{.addHealth = .{
				...
				.health = self.health,
				.cause = self.cause,
				.previous = if(side == .server) target.?.getHealth().health else main.game.Player.super.health,
			}}, side);
		}
