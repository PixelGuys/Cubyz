...
pub const Command = struct { // MARK: Command
	...
	fn do(self: *Command, allocator: NeverFailingAllocator, side: Side, user: ?*main.server.User, gamemode: main.game.Gamemode) error{serverFailure, Invalid}!void { // MARK: do()
	...
	const Deposit = struct { // MARK: Deposit
		...
		fn run(self: Deposit, allocator: NeverFailingAllocator, cmd: *Command, side: Side, user: ?*main.server.User, gamemode: Gamemode) error{serverFailure, Invalid}!void {
			if(self.source.inv.type != .normal and (self.source.inv.type != .creative or self.dest.inv.type != .normal)) return error.Invalid;
