...
pub const ClientInventory = struct { // MARK: ClientInventory
	...
	pub fn breakBlock(self: ClientInventory, slot: u32, deltaTime: f64, mods: main.Window.Key.Modifiers) void {
		std.debug.assert(self.type == .serverShared);
		main.renderer.MeshSelection.breakBlock(self, slot, deltaTime, mods);
	}
