...
pub fn init(zon: main.ZonElement) ?*@This() {
	...
	result.* = .{
		.block = main.blocks.parseBlock(zon.get(?[]const u8, "block", null) orelse {
			std.log.err("Missing field \"block\" for replaceBlockType event", .{});
			return null;
		}),
	};
	return result;
...
pub fn run(self: *@This(), params: main.callbacks.ServerBlockCallback.Params) main.callbacks.Result {
	...
	if (params.block.mode() != self.block.mode()) {
		std.log.err("{s} cannot replace {s} as they have different rotation modes", .{self.block.id(), params.block.id()});
		return .ignored;
	}

	self.block.data = params.block.data;
