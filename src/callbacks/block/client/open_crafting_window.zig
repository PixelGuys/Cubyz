...
pub fn init(zon: ZonElement, _: main.callbacks.Creator) ?*@This() {
	...
	if (craftingTags.len == 0) std.log.err("Missing craftingTags \"name\" for open_crafting_window event.", .{}) 
	result.* = .{
...
pub fn run(self: *@This(), _: main.callbacks.ClientBlockCallback.Params) main.callbacks.Result {
	main.gui.windowlist.inventory_crafting.openFromCallback(craftingTags)
