...
pub const TextBuffer = struct { // MARK: TextBuffer
	...
	pub fn init(allocator: NeverFailingAllocator, text: []const u8, initialFontEffect: FontEffect, showControlCharacters: bool, alignment: Alignment) TextBuffer {
		...
		for(self.glyphs, 0..) |*glyph, i| {
			glyph.x_advance = @as(f32, @floatFromInt(glyphPositions[i].x_advance))/TextRendering.font_unit_per_pixel;
			glyph.y_advance = @as(f32, @floatFromInt(glyphPositions[i].y_advance))/TextRendering.font_unit_per_pixel;
			glyph.x_offset = @as(f32, @floatFromInt(glyphPositions[i].x_offset))/TextRendering.font_unit_per_pixel;
			glyph.y_offset = @as(f32, @floatFromInt(glyphPositions[i].y_offset))/TextRendering.font_unit_per_pixel;
...
const TextRendering = struct { // MARK: TextRendering
	...
	var font_unit_per_pixel: f32 = undefined;
	...
	fn init() !void {
		...
		font_unit_per_pixel = @as(f32, @floatFromInt(freetypeFace.*.units_per_EM)) / @as(f32, @floatFromInt(textureHeight));
