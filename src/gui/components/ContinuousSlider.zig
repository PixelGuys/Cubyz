...
inline fn bar_pos(self: *ContinuousSlider) Vec2f {
	return Vec2f{1.5*border + self.button.size[0]/2, self.button.pos[1] + self.button.size[1]/2 - border};
}

inline fn bar_size(self: *ContinuousSlider) Vec2f {
	const range: f32 = self.size[0] - 3*border - self.button.size[0];
...
pub fn mainButtonPressed(self: *ContinuousSlider, mousePosition: Vec2f) main.callbacks.Result {
	...
	} else if (GuiComponent.contains(self.bar_pos(), self.bar_size(), mousePositionRelativeToSelf)) {
		self.mouseAnchor = self.button.size[0]/2;
...
pub fn render(self: *ContinuousSlider, mousePosition: Vec2f) void {
	...
	draw.rect(self.pos + self.bar_pos(), self.bar_size());
	...
	if (self.button.pressed) {
		...
		self.button.pos[0] = @min(@max(self.button.pos[0], 1.5*border), 1.5*border + self.bar_size()[0] - 0.001);
