...
pub fn onOpen() void {
	...
	list.add(Button.initText(.{0, 0}, 192, if (editingKeyboard) "Gamepad" else "Keyboard", .init(toggleKeyboard)));
	...
	for (&main.KeyBoard.keys) |*key| {
		const label = Label.init(.{0, 0}, 192, key.name, .left);
		const button = if (key == selectedKey) (Button.initText(.{16, 0}, 192, "...", .{})) else (Button.initText(.{16, 0}, 192, if (editingKeyboard) key.getName() else key.getGamepadName(), if (editingKeyboard) .initWithPtr(keyFunction, key) else .initWithPtr(gamepadFunction, key)));
