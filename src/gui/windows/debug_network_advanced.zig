...
fn renderConnectionData(conn: *main.network.Connection, name: []const u8, y: *f32) void {
	...
	draw.print("{s} | RTT = {d:.1} ms | {d:.0} kiB/RTT | {d} MTU estimate | pS: {t}", .{name, conn.rttEstimate/1000.0, conn.bandwidthEstimateInBytesPerRtt/1024.0, conn.mtuEstimate, conn.mtuProbingState}, 0, y.*, 8);
	y.* += 8;
