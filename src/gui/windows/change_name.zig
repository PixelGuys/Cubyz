...
pub fn onOpen() void {
	...
	if(settings.playerName.len == 0) {
		...
		window.titleBarButtons = &.{GuiWindow.zoomInButton, GuiWindow.zoomOutButton};
	} else {
		list.add(Label.init(.{0, 0}, width, "#ff0000Warning: #ffffffYou lose access to your inventory data when changing the name!", .center));
		window.titleBarButtons = &.{GuiWindow.zoomInButton, GuiWindow.zoomOutButton, GuiWindow.closeButton};
	}
