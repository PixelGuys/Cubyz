...
pub fn onOpen() void {
    ...
    for (worldList.items, 0..) |worldInfo, i| {
        ...
        row.add(Button.initIcon(.{ 4, 0 }, .{ 16, 16 }, editIcon, .{ .onAction = callback: {
            main.gui.windowlist.edit_world.setEditWorldName(worldList.items[i].fileName);
            break :callback main.gui.openWindowCallback("edit_world");
        } }));
