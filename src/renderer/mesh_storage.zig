...
fn batchUpdateBlocks() void {
    ...
    while (blockUpdateList.popFront()) |blockUpdate| {
        ...
        if (getMesh(pos)) |mesh| {
            ...
            std.log.debug("Block update received: old={s}({d}), new=({d}) at ({d},{d},{d})", .{ oldBlock.id(), oldBlock.typ, blockUpdate.newBlock.typ, blockUpdate.x, blockUpdate.y, blockUpdate.z });
