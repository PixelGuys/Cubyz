...
pub const ChannelChunk = struct {
	...
	fn propagateDirect(self: *ChannelChunk, lightQueue: *main.utils.CircularBufferQueue(Entry), lightRefreshList: *main.List(chunk.ChunkPosition)) void {
		...
		self.lock.lockWrite();
		while(lightQueue.popBack()) |entry| {
			...
			for(chunk.Neighbor.iterable) |neighbor| {
				...
				if(result.value[0] != 0 or result.value[1] != 0 or result.value[2] != 0) lightQueue.pushFront(result);
			}
		}
	...
	pub fn propagateLights(self: *ChannelChunk, lights: []const [3]u8, comptime checkNeighbors: bool, lightRefreshList: *main.List(chunk.ChunkPosition)) void {
		...
		for(lights) |pos| {
			...
			if(self.isSun) {
				lightQueue.pushFront(.{.x = @intCast(pos[0]), .y = @intCast(pos[1]), .z = @intCast(pos[2]), .value = .{255, 255, 255}, .sourceDir = 6, .activeValue = 0b111});
			} else {
				lightQueue.pushFront(.{.x = @intCast(pos[0]), .y = @intCast(pos[1]), .z = @intCast(pos[2]), .value = extractColor(self.ch.data.getValue(index).light()), .sourceDir = 6, .activeValue = 0b111});
			}
		}
		if(checkNeighbors) {
			for(chunk.Neighbor.iterable) |neighbor| {
				...
				while(x1 < chunk.chunkSize) : (x1 += 1) {
					...
					while(x2 < chunk.chunkSize) : (x2 += 1) {
						...
						if(value[0] != 0 or value[1] != 0 or value[2] != 0) lightQueue.pushFront(.{.x = @intCast(x), .y = @intCast(y), .z = @intCast(z), .value = value, .sourceDir = neighbor.toInt(), .activeValue = 0b111});
					}
				}
			}
		}
	...
	pub fn propagateLightsDestructive(self: *ChannelChunk, lights: []const [3]u8, lightRefreshList: *main.List(chunk.ChunkPosition)) void {
		...
		for(lights) |pos| {
			const index = chunk.getIndex(pos[0], pos[1], pos[2]);
			lightQueue.pushFront(.{.x = @intCast(pos[0]), .y = @intCast(pos[1]), .z = @intCast(pos[2]), .value = self.data.getValue(index), .sourceDir = 6, .activeValue = 0b111});
		}
		...
		for(constructiveEntries.items) |entries| {
			...
			for(entryList.items) |entry| {
				...
				lightQueue.pushFront(.{.x = entry.x, .y = entry.y, .z = entry.z, .value = value, .sourceDir = 6, .activeValue = 0b111});
			}
