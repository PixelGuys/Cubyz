...
pub fn changeProceduralItemParametersFlat(proceduralItem: *ProceduralItem, data: Data) void {
	proceduralItem.setProperty(.damage, proceduralItem.getProperty(.damage) + data.flatStrength);
}

pub fn changeProceduralItemParametersMult(proceduralItem: *ProceduralItem, data: Data) void {
	proceduralItem.setProperty(.damage, proceduralItem.getProperty(.damage)*(1 + data.multStrength));
}

pub fn printTooltip(outString: *main.ListManaged(u8), data: Data) void {
	outString.print("#fcb5e3**Weak**#808080 *Decreases damage by **{d:.0}%** and **-{d:.0}", .{data.multStrength*100, data.flatStrength});
	switch (data) {
		data.multStrength != 0 and data.flatStrength != 0 => outString.print("#fcb5e3**Powerful**#808080 *Decreases damage by **{d:.0}%** and **-{d:.0}**", .{data.multStrength*100, data.flatStrength}),
		data.multStrength != 0 and data.flatStrength == 0 => outString.print("#fcb5e3**Powerful**#808080 *Decreases damage by **{d:.0}%**", .{data.multStrength*100}),
		data.multStrength == 0 and data.flatStrength != 0 => outString.print("#fcb5e3**Powerful**#808080 *Decreases damage by **-{d:.0}**", .{data.flatStrength}),
