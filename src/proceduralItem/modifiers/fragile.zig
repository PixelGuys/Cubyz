...
pub fn changeProceduralItemParameters(proceduralItem: *ProceduralItem, data: Data) void {
	proceduralItem.setProperty(.maxDurability, @max(proceduralItem.getProperty(.maxDurability) - data.flatStrength, 0)*(1 - data.multStrength));
}

pub fn printTooltip(outString: *main.ListManaged(u8), data: Data) void {
	outString.print("#ccddff**Fragile**#808080 *Decreases durability by **{d:.0}%** and **-{d:.0}**", .{data.multStrength*100, data.flatStrength});
}
