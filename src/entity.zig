...
pub const ClientEntityManager = struct {
	...
	pub fn renderNames(projMatrix: Mat4f, playerPos: Vec3d) void {
		...
		for(entities.items()) |ent| {
			...
			const dstclamped :f32 = @as(f32, @floatCast((std.math.clamp(distance, 0, 100)+32)/32));
			...
			defer buf.deinit();
			const size = buf.calculateLineBreaks(48/dstclamped, 1024);
			buf.render(xCenter - size[0]/2, yCenter - size[1], 48/dstclamped);
		}
	}
