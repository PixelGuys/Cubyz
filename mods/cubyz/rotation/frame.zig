...
pub fn createBlockModel(_: Block, _: *u16, zon: ZonElement) ModelIndex {
	...
	if(!std.mem.eql(u8, modelId, "cubyz:cube")) {
		std.log.err("Frames can only be use on cube models, found '{s}'", .{modelId});
	}
	...
	for(0..len) |i| {
		...
		for (0..4) |j| {
			for (0..3) |k| {
				quadList.items[quadList.items.len - 1].corners[j][k] = (quadList.items[quadList.items.len - 1].corners[j][k] - 0.5) * 1.001 + 0.5;
			}
		}
...
pub fn canBeChangedInto(oldBlock: Block, newBlock: Block, _: main.items.ItemStack, shouldDropSourceBlockOnSuccess: *bool) RotationMode.CanBeChangedInto {
	...
	if(!oldBlock.transparent()) return .no;
