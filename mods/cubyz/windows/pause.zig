...
pub fn onOpen() void {
	...
	list.add(Button.initText(.{0, 0}, 128, "Players", .{.onAction = gui.openWindowCallback("cubyz:players"), .disabled = isSingleplayer}));
	if (main.server.world != null) {
		list.add(Button.initText(.{0, 0}, 128, "Invite Player", .{.onAction = gui.openWindowCallback("cubyz:invite"), .disabled = isSingleplayer}));
	}
