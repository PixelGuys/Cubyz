...
public class CustomBlock extends Ore implements CustomObject {
	...
	private static void readOreData() {
		try {
			InputStream ois = CustomBlock.class.getClassLoader().getResourceAsStream("io/cubyz/storage/custom_ore_names.dat");
			if (ois == null)
				ois = CustomBlock.class.getClassLoader().getResourceAsStream("classes/io/cubyz/storage/custom_ore_names.dat");
	...
	public static CustomBlock random(Random rand, CurrentSurfaceRegistries registries, TextureProvider texProvider) {
		...
		CustomBlock ore = new CustomBlock(rand.nextInt(200) - 100, (1+rand.nextFloat()*15)/16, 1+rand.nextFloat()*9, texProvider);
		ore.name = name;
